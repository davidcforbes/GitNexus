...
function publishGist(htmlPath: string): { url: string; rawUrl: string } | null {
  try {
    // GitNexus-dxu: emit `--` before htmlPath so a path beginning with `-`
    // (e.g. derived from a user-supplied repoPath) can't be parsed by `gh`
    // as an option flag and silently change gist visibility / behaviour.
